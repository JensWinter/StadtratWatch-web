...
/** The stable S3 prefix the paper batches live under, shared with the web build's reads. */
const PAPER_ASSETS_PREFIX = 'web-assets/papers';

/**
 * Publishes an authoritative batch set to `web-assets/papers/` and confirms the result. The assets
 * are the generator's complete picture of the prefix (it prunes stale batches as it writes), so the
 * publisher uploads new or changed batches, prunes orphans against them, and invalidates the touched
 * paths. A real publish then verifies the remote prefix against that same list, catching a silent put
 * or delete failure the plan alone would not reveal. A dry run reports the diff and skips both
 * mutation and verification. Assets and operations are passed in, so this is unit-testable without
 * writing to S3 or the filesystem.
 */
export async function publishAndVerifyPaperAssets(
  assets: LocalAsset[],
 ...
 */
export async function pushPaperAssetsFromEnv(
  outputDir: string,
  options: { dryRun?: boolean } = {},
): Promise<WebAssetPlan> {
  ...
  return publishAndVerifyPaperAssets(assets, target, operations, options);
}

function logPlan(plan: WebAssetPlan, dryRun: boolean): void {
  const heading = dryRun ? 'Dry run - would publish paper assets:' : 'Published paper assets:';
  console.log(heading);
